...
#include "generated/v2_golden_dma_job.h"
...
#define V2_EXPECTED_VERSION 0x00020000U
...
static uint32_t input_buffer[V2_INPUT_PACKET_WORDS] __attribute__((aligned(64)));
static int32_t output_buffer[V2_OUTPUT_WORDS] __attribute__((aligned(64)));
...
static void print_v2_status(void)
{
    ...
    xil_printf("V2 status       = 0x%08x\r\n", status);
    xil_printf("V2 irq_status   = 0x%08x\r\n", irq_status);
    xil_printf("V2 error_code   = 0x%08x\r\n", error_code);
    xil_printf("V2 stream_state = 0x%08x\r\n", stream_state);
    xil_printf("V2 packet_words = %d\r\n", packet_words);

    xil_printf("V2 status decode:");
...
{
    xil_printf("V2 perf job cycles      = %d\r\n", cnn_read(REG_PERF_JOB_CYCLES));
    xil_printf("V2 perf packet cycles   = %d\r\n", cnn_read(REG_PERF_PACKET_CYCLES));
    xil_printf("V2 perf compute cycles  = %d\r\n", cnn_read(REG_PERF_COMPUTE_CYCLES));
    xil_printf("V2 perf prefetch cycles = %d\r\n", cnn_read(REG_PERF_PREFETCH_CYCLES));
    xil_printf("V2 perf layer0 cycles   = %d\r\n", cnn_read(REG_PERF_LAYER0_CYCLES));
    xil_printf("V2 perf layer1 cycles   = %d\r\n", cnn_read(REG_PERF_LAYER1_CYCLES));
    xil_printf("V2 perf layer2 cycles   = %d\r\n", cnn_read(REG_PERF_LAYER2_CYCLES));
    xil_printf("V2 perf input words     = %d\r\n", cnn_read(REG_PERF_INPUT_WORDS));
    xil_printf("V2 perf input stalls    = %d\r\n", cnn_read(REG_PERF_INPUT_STALLS));
    xil_printf("V2 perf output words    = %d\r\n", cnn_read(REG_PERF_OUTPUT_WORDS));
    xil_printf("V2 perf output stalls   = %d\r\n", cnn_read(REG_PERF_OUTPUT_STALLS));
}
...
static int cnn_wait_done(void)
{
    for (uint32_t i = 0; i < CNN_TIMEOUT; i++) {
        ...
        if ((status & STATUS_ERROR) != 0U) {
            xil_printf("[FAIL] V2 accelerator reported an error\r\n");
            print_v2_status();
            return -1;
        }
    ...
    xil_printf("[FAIL] V2 accelerator done timeout\r\n");
    print_v2_status();
    print_dma_status("DMA MM2S at V2 timeout", dma_read(DMA_MM2S_DMASR));
    print_dma_status("DMA S2MM at V2 timeout", dma_read(DMA_S2MM_DMASR));
    return -1;
}

static void prepare_buffers(void)
{
    for (uint32_t i = 0; i < V2_INPUT_PACKET_WORDS; i++) {
        input_buffer[i] = v2_input_packet_words[i];
    }

    for (uint32_t i = 0; i < V2_OUTPUT_WORDS; i++) {
...
static int compare_output(const int32_t *expected)
{
    ...
    if (mismatches != 0U) {
        xil_printf("[FAIL] V2 output mismatch count = %d\r\n", mismatches);
        return -1;
    }
...
static int run_v2_job(const char *name, uint32_t final_residual_enable)
{
    const int32_t *expected = final_residual_enable ?
        v2_expected_residual_words :
        v2_expected_no_residual_words;
    uint32_t input_bytes = V2_INPUT_PACKET_WORDS * sizeof(uint32_t);
    uint32_t output_bytes = V2_OUTPUT_WORDS * sizeof(int32_t);
    ...
    xil_printf("Starting v2 accelerator and AXI DMA...\r\n");
    ...
    xil_printf("DMA+V2 transfer cycles = %d\r\n", (uint32_t)cycles);
    xil_printf("DMA+V2 transfer usec   = %d\r\n", cycles_to_usec(cycles));
...
int main(void)
{
    xil_printf("\r\n");
    xil_printf("========================================\r\n");
    xil_printf(" Zynq V2 Image-to-Image CNN DMA Test\r\n");
    xil_printf("========================================\r\n");
    xil_printf("CNN v2 base address: 0x%08x\r\n", CNN_BASE);
    xil_printf("AXI DMA base address: 0x%08x\r\n", DMA_BASE);

    uint32_t version = cnn_read(REG_VERSION);
    xil_printf("V2 register version: 0x%08x\r\n", version);

    if (version != V2_EXPECTED_VERSION) {
        xil_printf("[FAIL] Unexpected v2 register version, expected 0x%08x\r\n",
                   V2_EXPECTED_VERSION);
        while (1) {
            sleep(1);
        }
    ...
    int rc = 0;
    rc |= run_v2_job("v2_golden_full_network_residual", 1U);
    rc |= run_v2_job("v2_golden_full_network_no_residual", 0U);

    if (rc == 0) {
        xil_printf("\r\n[PASS] V2 image-to-image DMA golden test passed\r\n");
    } else {
        xil_printf("\r\n[FAIL] V2 image-to-image DMA golden test failed\r\n");
    }
