...
module tb_cnn_accel_top_random;
...
`ifdef USE_COVERGROUPS
  covergroup cg_config @(posedge clk);
    cp_relu: coverpoint dut.relu_enable {
      bins off = {0};
      bins on  = {1};
    }

    cp_quant: coverpoint dut.quant_enable {
      bins off = {0};
      bins on  = {1};
    }

    cp_shift: coverpoint dut.quant_shift {
      bins zero = {0};
      bins low  = {[1:3]};
      bins high = {[4:6]};
    }
...
`endif
    ...
    begin
        ...
        for (int r = 0; r < MAX_H; r++) begin
          for (int c = 0; c < MAX_W; c++) begin
            unique case (scenario)
              0: begin
                input_mem[ic][r][c] = $signed((ic + r + c) & 8'h7f);
              end

              1: begin
                input_mem[ic][r][c] = 8'sd3;
              end

              2: begin
                input_mem[ic][r][c] = 8'sd127;
              end

              default: begin
                input_mem[ic][r][c] = $signed(rand_range(0, 31)) - 8'sd16;
              end
            endcase
          end
        end
      ...
      for (int oc = 0; oc < OC; oc++) begin
        unique case (scenario)
          ...
          default: begin
            bias_mem[oc] = $signed(rand_range(0, 255)) - 32'sd128;
          end
        endcase
        ...
        for (int ic = 0; ic < IC; ic++) begin
          for (int k = 0; k < KERNEL_TAPS; k++) begin
            unique case (scenario)
              ...
              1: begin
                weight_mem[oc][ic][k] = -8'sd2;
              end

              2: begin
                weight_mem[oc][ic][k] = 8'sd127;
              end

              default: begin
                weight_mem[oc][ic][k] = $signed(rand_range(0, 31)) - 8'sd16;
              end
            endcase
          end
        end
  ...
  task automatic compute_expected(
    ...
    begin
      if (kernel_mode_arg) begin
        ...
        // 1x1 convolution keeps full spatial size.
        out_h = h;
        out_w = w;
      end
  ...
  task automatic collect_and_check(
    input int total_outputs,
    ...
    begin
      ...
      if (timeout >= 30000) begin
        errors++;
        $error(
          "%s timed out after collecting %0d/%0d outputs. busy=%0b done=%0b output_count=%0d",
          name,
          idx,
          total_outputs,
          busy,
  ...
  task automatic wait_done(input string name);
    ...
    begin
      ...
      while (busy !== 1'b0 && timeout < 1000) begin
        @(posedge clk);
        timeout++;
      end
      ...
      if (timeout >= 1000) begin
        errors++;
        $error(
          "%s timed out waiting for busy low. busy=%0b done=%0b output_count=%0d",
          name,
          busy,
          done,
          output_count
  ...
  task automatic run_midstream_reset;
    begin
      $display("============================================================");
      $display("[SCENARIO] midstream_reset_recovery");
      $display("============================================================");

      reset_dut();

      init_scenario(3, 5, 5);
      program_dut(5, 5, 1'b1, 1'b1, 1'b1, 2);

      m_axis_tready = 1'b0;
      start_dut(1'b1, 1'b1, 1'b1);

      for (int n = 0; n < 10; n++) begin
        send_one_pixel(input_mem[0][n / 5][n % 5], 1'b0, 1'b0);
      end
      ...
      if (busy !== 1'b0 || m_axis_tvalid !== 1'b0 || done !== 1'b0) begin
        errors++;
        $error(
          "midstream reset did not clear status/output: busy=%0b valid=%0b done=%0b",
          busy,
          m_axis_tvalid,
          done
        );
      ...
      $display("[SCENARIO PASS] midstream_reset_recovery");
    end
  endtask
  ...
  initial begin
    $dumpfile("tb_cnn_accel_top_random.vcd");
    $dumpvars(0, tb_cnn_accel_top_random);
    ...
    run_scenario(
      "saturation_no_quant_with_output_backpressure",
      2,
      5,
      5,
      1'b0,
    ...
    run_scenario(
      "random_6x6_relu_quant_shift2_backpressure",
      3,
      6,
      6,
      1'b1,
    ...
    run_scenario(
      "random_8x6_relu_quant_high_shift",
      5,
      8,
      6,
      1'b1,
    ...
    run_scenario(
      "random_1x1_6x5_relu_bias_quant_backpressure",
      6,
      6,
      5,
      1'b1,
    ...
    $display("============================================================");
    $display("tb_cnn_accel_top_random summary");
    $display("Tests run : %0d", tests);
    $display("Errors    : %0d", errors);
    $display("============================================================");

    if (errors != 0) begin
      $fatal(1, "tb_cnn_accel_top_random FAILED: tests=%0d errors=%0d", tests, errors);
    end else begin
      $display("[PASS] tb_cnn_accel_top_random tests=%0d", tests);
    end
