...
module tb_v2_axi_lite_slave;
  ...
  initial begin
    ...
    if (errors == 0) begin
      $display("[PASS] tb_v2_axi_lite_slave tests=%0d", checks);
    end else begin
      $display("[FAIL] tb_v2_axi_lite_slave errors=%0d checks=%0d", errors, checks);
      $fatal(1);
    end
