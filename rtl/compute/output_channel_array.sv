  ...
  generate
    for (oc = 0; oc < NUM_OUTPUT_CHANNELS; oc++) begin : GEN_OC
      conv_engine #(
        ...
        .OUT_WIDTH(OUT_WIDTH), .BIAS_WIDTH(BIAS_WIDTH), .NUM_INPUT_CHANNELS(NUM_INPUT_CHANNELS), .KERNEL_TAPS(KERNEL_TAPS)
      ) u_conv_engine (
        .windows(windows), .weights(weights[oc]), .bias(bias[oc]),
        .relu_enable(relu_enable), .bias_enable(bias_enable), .quant_enable(quant_enable),
        .quant_shift(quant_shift), .acc_raw(acc_raw[oc]), .out_data(out_data[oc])
      );
    end
  endgenerate
endmodule
