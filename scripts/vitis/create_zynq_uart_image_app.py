...
root = Path(os.getcwd())
workspace = root / "build" / "vitis_uart_ws"
xsa_file = root / "build" / "arty_z7_20_cnn" / "arty_z7_20_cnn.xsa"

platform_name = "arty_z7_20_cnn_uart_platform"
app_name = "cnn_uart_image"
...
# Use a separate workspace so this does not affect the stable cnn_baremetal app.
if workspace.exists():
    print(f"Removing old UART Vitis workspace: {workspace}")
...
src_main = root / "software" / "zynq_uart_image" / "main.c"
...
print(f"Copied UART image app source to {dst_main}")
...
print("")
print("Vitis UART image app build done.")
print("ELF:")
print(elf)
print("")
