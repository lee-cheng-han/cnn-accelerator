...
SWEEP_ROOT="${SWEEP_ROOT:-build/v2_synth_sweep}"
...
for config in "${configs[@]}"; do
  ...
  echo "============================================================"
  echo "[V2 SYNTH] PC=${pc} PK=${pk}"
  echo "============================================================"

  PC="$pc" PK="$pk" OUT_DIR="$out_dir" \
    "$VIVADO_BIN" \
      -mode batch \
      -source scripts/v2/synth_pc_pk_config.tcl \
      -log "${out_dir}/vivado.log" \
      -journal "${out_dir}/vivado.jou"
done

python3 scripts/v2/report_synth_sweep.py \
  --sweep-root "$SWEEP_ROOT" \
  --markdown docs/v2_synthesis_experiments.md
