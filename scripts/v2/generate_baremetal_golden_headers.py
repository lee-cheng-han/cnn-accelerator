"""Generate C headers for the v2 bare-metal golden DMA test."""
...
def generate(case_dir: Path, out_dir: Path) -> None:
    ...
    header = out_dir / "v2_golden_dma_job.h"
    text = [
        "#ifndef V2_GOLDEN_DMA_JOB_H",
        "#define V2_GOLDEN_DMA_JOB_H",
        "",
        "#include <stdint.h>",
        "",
        f"#define V2_IMAGE_WIDTH {width}U",
        f"#define V2_IMAGE_HEIGHT {height}U",
        f"#define V2_OUTPUT_WIDTH {output_width}U",
        f"#define V2_OUTPUT_HEIGHT {output_height}U",
        f"#define V2_INPUT_PIXELS {pixels}U",
        f"#define V2_OUTPUT_WORDS {output_words}U",
        f"#define V2_INPUT_PACKET_WORDS {len(packets)}U",
        f"#define V2_FINAL_RESIDUAL_DEFAULT {config[CFG_FINAL_RESIDUAL_ENABLE]}U",
        "",
        format_array(packets, "uint32_t", "v2_input_packet_words"),
        "",
        format_array(expected_residual, "int32_t", "v2_expected_residual_words", per_line=8),
        "",
        format_array(expected_no_residual, "int32_t", "v2_expected_no_residual_words", per_line=8),
        "",
        "#endif",
        "",
    ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--case-dir",
        default="build/v2_golden/full_network_3layer",
        help="Generated v2 full-network golden case directory",
    )
    parser.add_argument(
        "--out-dir",
        default="software/zynq_v2_baremetal/generated",
