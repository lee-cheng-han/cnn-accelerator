#!/usr/bin/env python3
"""Summarize the v2 top-level out-of-context implementation experiment."""
...
def render_markdown(result: TopImplResult, build_dir: Path) -> str:
    ...
    checkpoint = "v2_top_routed.dcp" if routed else "v2_top_synth.dcp"
    ...
    lines = [
        ...
        "",
        "This is an out-of-context implementation experiment for the v2 image-to-image RTL top. "
    ...
    if routed and result.timing_met:
        lines.append(
            "The current v2 top fits, routes, and meets the 125 MHz internal clock target in this "
            "out-of-context smoke configuration."
        )
    elif routed:
        lines.append(
            "The current v2 top now fits and routes in this out-of-context smoke configuration, "
        ...
        lines.append(
            "The current v2 top synthesizes but does not yet fit the Arty Z7-20 target in this "
...
def main() -> int:
    ...
    parser.add_argument("--build-dir", type=Path, default=Path("build/v2_top_impl"))
    parser.add_argument("--markdown", type=Path, default=Path("docs/v2_top_implementation.md"))
