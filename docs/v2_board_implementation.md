...
The design is board-implementation-ready evidence, but it is not yet a complete board demo package. The next step is v2 bare-metal software that programs the v2 register map, sends the packetized tensor job through AXI DMA, reads back the output stream, and checks it against the golden tensor data.
